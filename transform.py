...
from const import DETPRON, SELFDETERMINERS, MYDETERMINERS, QUOTES, segmenter, morph_tagger, syntax_parser, ner_tagger, \
    BOTH, OPEN, CLOSE
from convert import make_replacement, change_case, name_to_case, verb_mapping
...
def analyze_quotes(doc):
    quotes = [False] * len(doc.tokens)
    current = 0
    for sent in doc.sents:
        is_quote = False
        prev_quote = None
        for token in sent.tokens:
            quote = QUOTES.get(token.text)
            if quote:
                if prev_quote is None and quote[1] in [OPEN, BOTH]:
                    prev_quote = quote
                    is_quote = True
                elif prev_quote[1] == BOTH and quote[1] == BOTH and prev_quote == quote:
                    # close it here
                    is_quote = False
                    prev_quote = None
                elif prev_quote[1] == OPEN and quote[1] == CLOSE and prev_quote[0] == quote[0]:
                    is_quote = -1
                    prev_quote = quote
                elif prev_quote[1] == CLOSE and quote[1] == CLOSE and prev_quote[0] == quote[0]:
                    i = current - 1
                    while quotes[i] == -1:
                        quotes[i] = True
                        i -= 1
                elif prev_quote[1] == CLOSE and quote[1] == OPEN:
                    i = current - 1
                    while quotes[i] == -1:
                        quotes[i] = False
                        i -= 1
                    prev_quote = quote
                    is_quote = True
            quotes[current] = is_quote
            current += 1
        i = current - 1
        while quotes[i] == -1:
            quotes[i] = prev_quote[1] == OPEN
            i -= 1
    return quotes
...
def find_ambiguous_pronouns(gender, doc, narrator, quotes):
    """
        ...
        for i in range(current_token, len(tokens)):
            ...
            if step == '1' and (token.feats.get("Person", None) == "1" or token.pos == "DET"):
                ...
                if token.pos == "PRON":
                    step = '3'
                    to_change[token.id] = (
                        token.start, token.stop, (name_to_case(narrator, gender,
                                                               token.feats.get('Case')) if token.feats.get('Case',
                                                                                                           'Nom') != 'Nom' else narrator).strip())
...
if __name__ == '__main__':
    ...
    #     "Гоша сказал: \"Я люблю тебя и «Сбербанк»\". Ипотечный договор я заключал с банком ЗАО «Коммерческий Банк «Зернобанк», поэтому я не смог там быть.",
    #     "Алексей Смоляков"))
    ...
    # print(transform_text("""В настоящее время при мне находится коробка от мобильного телефона марки
    ...
    # телефон, я в этом не виновата.""", "Алена"))
    # print(transform_text("""По существу заданных вопросов могу пояснить, что
    # по вышеуказанному адресу проживаю с родителями Совуновой Аллой Николаевной и
    # Совуновым Вадимом Евгеньевичем. В свободное от обучения в средней общеобразовательной школе № 96 время я занимаюсь в цирковой студии «Престиж», с которой периодически выступаю, в том числе с выездом.
    # Так, 11.05.2015 года я с напарником по цирковой студии Галич Денисом Юрьевичем
    ...
    # «Iphone» модель «4s», IMEI 013532009242428 в корпусе белого цвета, два электронных билета на листах А4, копия свидетельства о рождении на мое имя, а также электронная выписка чека из магазина «Связной» и документы из магазина, где приобретали мобильных
    # телефон, я в этом не виновата. Данные предметы и документы я готова выдать.
    # Больше по данному факту мне пояснить нечего. """, "Совунова Анастасия Вадимовна")[0])
...
#                          "Участник"))
    print(transform_text("""`Вчера я поехала к Алене на работу. Она была там со мной с 17:00 до 20:00. Мы поели в ресторане после работы, и потом Алена пошла домой. Когда я пришла домой, я ей позвонила, но она не ответила. Я начала переживать и поехала к ней. Она была уже мертва.""", "Алина Сергеева"))
